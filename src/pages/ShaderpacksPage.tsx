...
import { Trash2, FolderOpen, Sun, RefreshCw, Folder, Globe, Download, Search, Loader2, ChevronDown, ChevronRight, ExternalLink } from 'lucide-react'
...
export default function ShaderpacksPage() {
  ...
  useEffect(() => {
    loadPacks()
    window.electronAPI.settings.get().then(s => setDestinationDir(s.minecraftDirectory + '/shaderpacks'))
    handleSearch('')
  ...
  async function loadPacks() {
    setLoading(true)
    try {
      const list = await window.electronAPI.shaderPacks.list()
      setPacks(list)
    } catch {}
    setLoading(false)
  }
  ...
  async function handleDelete(path: string) {
    const result = await window.electronAPI.shaderPacks.delete(path)
    if (result.success) {
      setNotif({ message: 'Shader pack deleted', type: 'success' })
      loadPacks()
    } else {
  ...
  async function handleSearch(q?: string, versionOv?: string, loaderOv?: string, sortOv?: string) {
    const term = (q ?? query).trim()
    const vFilter = versionOv !== undefined ? versionOv : filterVersion
    const lFilter = loaderOv !== undefined ? loaderOv : filterLoader
    ...
    try {
      ...
      const loaders = lFilter ? [lFilter] : undefined
      const res = await window.electronAPI.modrinth.search(term, 'shader', 20, sFilter !== 'relevance' ? sFilter : undefined, versions, loaders)
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-header">
        <h1 className="page-title">Shader Packs</h1>
        <p className="page-subtitle">Browse Modrinth or manage installed shaders</p>
      </div>

      {/* Shader loader info banner */}
      <div style={{
        marginBottom: 12, padding: '10px 14px', fontSize: 12,
        background: 'var(--warning-dim)', borderLeft: '3px solid var(--warning)',
        display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap'
      }}>
        <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>⚠ Shader packs require a loader</span>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          <button className="btn btn-ghost btn-xs" onClick={() => window.electronAPI.shell.openExternal('https://irisshaders.dev')}
            style={{ color: 'var(--accent)', fontWeight: 600 }}>
            Iris <ExternalLink size={10} />
          </button>
          <span className="text-xs text-muted">|</span>
          <button className="btn btn-ghost btn-xs" onClick={() => window.electronAPI.shell.openExternal('https://optifine.net/downloads')}
            style={{ color: 'var(--accent)', fontWeight: 600 }}>
            OptiFine <ExternalLink size={10} />
          </button>
          <span className="text-xs text-muted">|</span>
          <button className="btn btn-ghost btn-xs" onClick={() => window.electronAPI.shell.openExternal('https://modrinth.com/mod/oculus')}
            style={{ color: 'var(--accent)', fontWeight: 600 }}>
            Oculus <span className="text-muted" style={{ fontWeight: 400 }}>(Forge/NeoForge)</span> <ExternalLink size={10} />
          </button>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="flex items-center gap-2" style={{ padding: '8px 12px' }}>
          ...
          <input className="input" style={{ flex: 1, border: 'none', background: 'transparent', padding: '4px 0' }}
            placeholder="Search shader packs on Modrinth..."
          ...
          <button className="btn btn-secondary btn-sm" onClick={() => window.electronAPI.shaderPacks.openFolder()}>
        ...
        <div className="flex items-center gap-2" style={{ padding: '4px 12px 8px' }}>
          ...
          <select className="input" style={{ width: 130, fontSize: 12, padding: '3px 6px' }} value={filterLoader}
            onChange={e => { const v = e.target.value; setFilterLoader(v); handleSearch(undefined, undefined, v) }}>
            {SHADER_LOADERS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
          <select className="input" style={{ width: 140, fontSize: 12, padding: '3px 6px' }} value={sortIndex}
            onChange={e => { const v = e.target.value; setSortIndex(v); handleSearch(undefined, undefined, undefined, v) }}>
        ...
        {!searching && searched && results.length === 0 && (
          <div style={{ padding: 24, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
            No shader packs found. Try different filters.
          </div>
        )}
      ...
      {showInstalled && (
        loading ? (
          ...
          <div className="card" style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>
            <Sun size={24} style={{ margin: '0 auto 8px', opacity: 0.3 }} />
            <div style={{ fontSize: 13 }}>No packs installed. Search and install from Modrinth above.</div>
            <button className="btn btn-secondary btn-sm" style={{ marginTop: 12 }} onClick={() => window.electronAPI.shaderPacks.openFolder()}>
              <FolderOpen size={14} /> Open Shaderpacks Folder
            </button>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {packs.map((pack, i) => (
              <div key={i} className="card" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px' }}>
                <Sun size={16} style={{ color: 'var(--accent)', flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this shader pack? This cannot be undone."
